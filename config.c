...
int
config_set_default(Configuration * config)
{
	if(config == NULL)
		return -1;
...
static int
parse_handler(void *user, const char *section, const char *name,
	      const char *value)
{
...
int
socket_connect(char *host, u_short port)
{
	...
	if(connect(sock, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
		log_debug(logger, "socket connect error[%d]: %s", errno,
			  strerror(errno));
		return -errno;
	}
